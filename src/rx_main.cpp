...
bool ICACHE_RAM_ATTR HandleSendTelemetryResponse()
{
    ...
    WORD_ALIGNED_ATTR uint8_t data[5] = {(uint8_t)(rand() % 255), (uint8_t)(rand() % 255), (uint8_t)(rand() % 255), (uint8_t)(rand() % 255), (uint8_t)(rand() % 255)};
    WORD_ALIGNED_ATTR uint8_t fulldata[10] = {(uint8_t)(rand() % 255), (uint8_t)(rand() % 255), (uint8_t)(rand() % 255), (uint8_t)(rand() % 255), (uint8_t)(rand() % 255), (uint8_t)(rand() % 255), (uint8_t)(rand() % 255), (uint8_t)(rand() % 255), (uint8_t)(rand() % 255), (uint8_t)(rand() % 255)};
    alreadyTLMresp = true;
    // otaPkt.std.type = PACKET_TYPE_TLM;
    otaPkt.std.type = PACKET_TYPE_SYNC;
    otaPkt.std.sync.UID3 = UID[3];
    otaPkt.std.sync.UID4 = UID[4];
    otaPkt.std.sync.UID5 = UID[5];
    ...
    //     if (OtaIsFullRes)
    //     {
    //         // otaPkt.std.tlm_dl.packageIndex = rand() % 255;
    //         // memcpy(&(otaPkt.std.tlm_dl.payload), &fulldata, sizeof(otaPkt.std.tlm_dl.payload));
    //         otaPkt.full.tlm_dl.packageIndex = TelemetrySender.GetCurrentPayload(
    ...
    //         // otaPkt.std.tlm_dl.packageIndex = rand() % 255;
    //         // memcpy(&(otaPkt.std.tlm_dl.payload), &data, sizeof(otaPkt.std.tlm_dl.payload));
...
bool ICACHE_RAM_ATTR MyProccessRFPacket(SX12xxDriverCommon::rx_status const status)
{
    ...
    //     char *str;
    //     str = (char*)malloc(20);
    //     int l = sprintf(str, "CRC Valid Error\n");
    //     Serial.write(str, l);
    //     free(str);
    //     // return false;
    // }
    ...
    if (otaPktPtr->std.type == PACKET_TYPE_SYNC)
    {
        ...
        char *str;
        str = (char*)malloc(20);
        int l = sprintf(str, "Got Sync Packet\n");
        Serial.write(str, l);
        free(str);
...
bool ICACHE_RAM_ATTR ProcessRFPacket(SX12xxDriverCommon::rx_status const status)
{
    ...
    // call = (uint8_t *)Radio.RXdataBuffer;
    // char *str;
    // int l = sprintf(str, "%d, %d, %d, %d\n", call[0], call[1], call[2], call[3]); 
...
{
    return MyProccessRFPacket(status);
...
//     if (now - last > 1)
//     {
        // Radio.TXnb((uint8_t *)&call, sizeof(uint8_t) * 4);
        // call[0] = (uint8_t)rand() % 0xFF;
        // call[1] = (uint8_t)rand() % 0xFF;
        // call[2] = (uint8_t)rand() % 0xFF;
        // call[3] = (uint8_t)rand() % 0xFF;
//         last = now;
//    }
    // HandleSendTelemetryResponse();
